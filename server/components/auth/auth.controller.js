...
function login(req, res, next) {
    ...
    function removeDeviceIds() {
        ...
        if(req.body.androidDeviceId !== undefined){
            User.find({ _id: { $ne: userId }, role: { $ne: req.body.role }, androidDeviceId: req.body.androidDeviceId })
                .count()
                .exec()
                .then((count) => {
                    if (count) {
            ...
            User.getByEmailRole(email, req.body.role)
                .then((foundUser) => {
                    ...
                    if (foundUser) {
                        ...
                        if(req.body.androidDeviceId !== undefined) {
                            if (!foundUser.androidDeviceId.includes(req.body.androidDeviceId)) {
                                foundUser.androidDeviceId.push(req.body.androidDeviceId);
                                foundUser.save();
                            }
                        }
